...
Before implementing Pass 2 I did a Phase A pass on the cited references. Phase A in my workflow means: read what the practitioners actually shipped, not what the research summary claims they shipped. The reasoning is older than my framework but the framework has a specific name for the pattern this catches.

Codrops's article (*"How to Create a PS1-Inspired Jitter Shader with React-Three-Fiber"*, published 2024-09-03) does not use a custom `ShaderMaterial` from scratch. It uses Three.js's `onBeforeCompile` hook to patch the shader source of the built-in `MeshLambertMaterial` at compile time. The patch injects vertex-snap math into the existing vertex shader and adjusts the lighting interpolation to behave as flat per-vertex. The base material's lighting pipeline, asset binding, GLB-material compatibility, and shader-permutation handling all stay intact; the patch surgically modifies the parts that need to be different.

Roman Liutikov's Three.js PS1 work uses the same pattern. Same `onBeforeCompile` hook, same patch-strategy against the built-in material, same survival of the base material's downstream behavior. Two independent practitioners, neither citing the other, converging on the same concrete implementation. Different from Grok's verdict, which had stated they used custom `ShaderMaterial` from scratch.
...
The discipline that made the decision land cleanly was the Phase A surface check: before implementing, read the actual practitioner code that the research verdict cites. The Phase A check is a few minutes of work and it caught a divergence that would have shaped every Pass 2 commit downstream. Without it, Grok's verdict would have read as authoritative and I would have implemented the harder pattern.

## The result

Pass 2 went straight. The execution arc:
