...
post_id: 0191
title: When Six Rounds of Substrate Research Canonized Schema Architecture v0.2 with Eleven Life Aspects, CIDOC-CRM, and First-Class Intersection Nodes
...
mechanism_details:
- Six-round substrate arc sequence: Round 1 Grok Expert seven shelf additions plus five structural patterns; Round 2 Claude Opus Extended Thinking five architectural pivots plus fifteen live-verified academic citations; Round 3 Claude Web live-fetch cross-verification plus economy simulation architecture; Round 4 ChatGPT 5.5 via Perplexity AUTHORING-vs-RUNTIME distinction plus transmission functions plus six-layer economy plus ten bowling event schemas plus compiled epistemic state; Round 5 empirical Getty AAT SPARQL probe plus eleven-aspect life-modeling matrix plus eleven compiled runtime tables plus five-attribute minimum row schema; Round 6 Tier-1 evidence upgrades (Cosmic Bowling trademark Registration 2702269 plus 1000xRESIST Peabody plus Consalvo Cheating ISBN 9780262033657 plus HABS public-domain confirmation) plus Schwartz values backbone plus Homosaurus plus ULAN correction
- CIDOC-CRM specifics all live-verified: ISO 21127:2023 third edition supersedes 2014; v7.1.1 May 2021 equals 81 classes plus 160 properties; DOI 10.26225 FDZH X261; event-centric core E5 Event plus E52 Time-Span plus E53 Place plus E39 Actor plus E77 Persistent Item plus E2 Temporal Entity plus E92 Spacetime Volume plus E3 Condition State plus E7 Activity plus E29 Design or Procedure
- Eleven life-aspects with modeling pattern plus controlled vocabulary plus era variation plus game precedent plus consultation gate: needs and drives, values goals aspirations (Schwartz backbone), habits routines, group memberships, health body condition-state, financial position, language dialect, beliefs, communication style interaction norms, consumption material possessions, time-use patterns
- Eight-vocabulary supplement stack: O NET (1016 titles plus 923 data-level plus 55000 jobs; CC BY 4.0) plus MusicBrainz plus Wikidata plus VIAF plus LC Name Authorities plus LCSH FAST plus Homosaurus plus ISO 639-2 Glottolog Ethnologue plus ATUS AHTUS plus ICD plus Pew Religious Landscape Study
- Consultation gate as first-class policy object compiles to runtime permissions (access plus display plus transformation plus reuse rules); frameworks CARE plus OCAP plus Local Contexts TK Labels plus Mukurtu plus UNESCO ICH plus OHA plus APA Ethics plus Never Alone paradigm case (Upper One Games plus E-Line Media 2014 approximately 40 Alaska Native elders plus Inupiat Cultural Ambassadors)
...
- Canonical: architecture/schema-architecture-v0.2-canonical-2026-07-07.md
CC-OPTIMIZED-SUMMARY -->

# When Six Rounds of Substrate Research Canonized Schema Architecture v0.2 with Eleven Life Aspects, CIDOC-CRM, and First-Class Intersection Nodes

**Author:** Jimmy McColery
**Date:** 2026-07-07
**Status:** Multi-substrate research arc closure, n=3 unique substrate families across six substrate runs, all contributions graded A/A+ per Post 0163 six-dimension rubric
...
I decided to run a multi-substrate research arc to figure out what schema architecture would actually work at the scale I need. Six rounds later, I have a canonical architecture that fixes the shelf problem and introduces four new patterns that were not on my radar when I started.

This post documents the arc close.

## The arc structure

I ran six rounds across three unique substrate families over about a week. Each round consumed the prior rounds as input, cross-checked prior findings, and added its own contributions.

Round 1 was Grok Expert. Grok surfaced seven new shelf additions the initial architecture missed (Dynamic Emotion, Life Stages, Skills, Social Networks, Migration, Family Kinship, Ideology), five structural patterns, eleven research sources, and five representation pitfalls. Grok also flagged the bottom-line trap: "authenticity theater without descendant consultation."

Round 2 was Claude Opus Extended Thinking. Opus proposed five load-bearing architectural pivots. First, the shelves are non-MECE (mutually exclusive collectively exhaustive) because they conflate three different kinds of thing (indexing axes, subject domains, cross-cutting facets). Second, adopt CIDOC-CRM as backbone ontology because it has event-centric E-classes that map cleanly onto the shelf-fixing distinction. Third, add first-class intersection nodes for compound identity per Crenshaw 1989/1991 intersectionality framing. Fourth, use BIBFRAME Work → Instance → Item pattern for cultural bundle versioning. Fifth, formalize consultation gate as hard dependency. Opus verified fifteen academic citations live-fetch during the session.

Round 3 was a Claude Web live-fetch verification pass. Round 3 cross-verified Opus's five pivots (four corroborated, one mixed evidence) and introduced the economy simulation architecture question (how does a bowling alley tycoon handle nested economies?). Round 3's answer: hybrid local ABM plus exogenous historical shocks (the RimWorld incident pattern elevated with historical-fact data).

Round 4 was ChatGPT 5.5 via Perplexity. Round 4 introduced the AUTHORING-vs-RUNTIME distinction as a load-bearing pattern. It also introduced transmission functions (explicit named rules for converting macro facts into local variables), a six-layer economy architecture, ten named bowling event schemas, the compiled epistemic state pattern (Bad News lineage), and the idea of cultural protocols as runtime permissions rather than boolean approval flags.
...
## The authoring-runtime distinction

I did not anticipate this pattern. Round 4 raised it, Round 5 empirically probed it, Round 6 canonized it.

The problem: CIDOC-CRM is a beautiful ontology (81 classes, 160 properties, event-centric, ISO-standard). It is also computationally expensive. Real deployments (British Museum, Rijksmuseum, Museo del Prado) use RDF triple-stores that answer SPARQL queries in the low hundreds of milliseconds. That is fine for a museum catalog. It is not fine for a game tick loop running one thousand to five thousand NPCs per frame.

The ResearchSpace CRM-FR search paper (verified live during Round 6) has a memorable phrase for querying CIDOC-CRM graphs at scale: the number of possible query combinations is "staggering."

The fix: separate the authoring layer from the runtime layer. Author historically and ethically sensitive facts as E-entities plus typed properties with source, evidence, and consultation-policy provenance in the CIDOC-CRM plus PROV-O authoring graph. Attach controlled-vocabulary identifiers (SKOS plus Getty AAT plus the eight-vocabulary supplement stack) at the vocabulary layer. Compile authored content into eleven denormalized runtime tables (npc_drive_state, npc_value_profile, npc_routine_template, and so on) that the gameplay loop reads at O(1). Enforce consultation-derived permissions (access, display, transformation, reuse) as runtime rules at the compile-target.

The authoring graph is where the humanities work happens. The runtime tables are where the game engine runs. The compilation step is where they meet.
...
Round 6 recommended Schwartz Theory of Basic Human Values as a values backbone analogous to HEXACO for personality. Ten basic values (self-direction, stimulation, hedonism, achievement, power, security, conformity, tradition, benevolence, universalism) organized in a quasi-circumplex reducible to four higher-order dimensions (Openness to Change, Conservation, Self-Transcendence, Self-Enhancement). Validated instruments include PVQ-RR (Portrait Values Questionnaire Revised) and HOVS17 (Higher-Order Values Survey).

Round 6 also recommended treating Kohlberg moral development as historical context only, not as a spine. Kohlberg has known problems (Gilligan gender-bias critique, cultural-universality challenges). Schwartz values plus Haidt Moral Foundations are better-validated replacements.
...
A six-layer economy simulation architecture with named transmission functions and ten bowling event schemas anchored to real historical dates and trademarks. Data source verification for NBER, FRED, BLS, BEA, IPUMS USA, NHGIS, HSUS Millennial Edition, Census CBP, HABS/HAER (46000 sites public domain), USBC, Bowlero, and cosmic bowling attribution.
...
What I am claiming is this: I ran six rounds of substrate research across three unique substrate families with per-round Post 0163 six-dimension rubric applied. The convergent architecture handles the non-MECE shelf problem, introduces four new patterns (AUTHORING-vs-RUNTIME compilation, first-class intersection nodes, Schwartz values backbone, compiled epistemic state) that were not on my radar when I started, and preserves a Rule 24 audit record acknowledging the convergence-is-not-truth caveat. That is the arc close. The next arcs are implementation.
