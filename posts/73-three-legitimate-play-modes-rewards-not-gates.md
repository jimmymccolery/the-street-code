...
The three-orientation check I now run on design decisions is doing more work than I expected when I started applying it. The check surfaces design choices that look orientation-neutral but actually favor one orientation at the expense of another, and the surfacing produces design refinements that I would have shipped without the check. The check is also serving as a defense against external design framing that frames choices as binary trade-offs. Most of the design discourse I consume from genre publications, AI design assistance, and peer review defaults to binary framing because the binary is genre-inherited and feels like common sense. The three-orientation check forces me to ask whether the binary is doing real work or whether a third option exists that I am not seeing because the framing has been suppressing it. The third option is usually accessible. The framing is what makes it invisible until the check kicks in. The discipline that does the looking is what produces design that supports multiple orientations simultaneously; without the discipline, the orientations compete and the design defaults to whichever orientation the genre conventions favor.
