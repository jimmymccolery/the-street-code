...
The architecture I now have is doing more work than I expected when I sketched it. It is solving the federation-tournament feature without infrastructure I would have had to maintain in perpetuity. It is producing achievement records that will outlast my involvement with the project. It is reframing my own assumptions about which features require central infrastructure and which features can be built from public-domain primitives that are already mature. The reframing is the secondary lesson. The central-server assumption was not load-bearing in the architecture; the assumption was inherited from genre convention and was sitting inside my design thinking as a constraint I had not examined. Examining the assumption produced an architecture that fits my actual operational constraints better than the genre convention would have. The architecture also fits the privacy properties I care about better than the central-server alternative would have. Both fits emerge from rejecting one inherited assumption that turned out to be optional. The operator-directed scan that surfaced the existing scattered infrastructure was what made the rejection possible; without the scan, the assumption would have continued to constrain the design indefinitely.
